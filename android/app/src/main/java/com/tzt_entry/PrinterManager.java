...
//   }
  ...
  void printImageFromDrawable(ReactApplicationContext context, int image) {
    ...
    if (mService.getState() == BluetoothService.STATE_CONNECTED) {
      ...
      Bitmap bm = BitmapFactory.decodeResource(context.getResources(), R.drawable.printer);
      if (bm != null) {
  ...
  @ReactMethod
  private void printImageFromFilePath(ReactApplicationContext context, String path,int width) {
    ...
    if (bm != null) {
     ...
     bm = Bitmap.createScaledBitmap(bm, width, height, false);
      ...
      sendData = PrintPicture.POS_PrintBMP(bm, width, 0);
      mService.write(sendData);
    } else {
      Toast.makeText(context, Constants.no_file, Toast.LENGTH_SHORT).show();
    }
  ...
  @ReactMethod
  void printImage(String imagePath,int width, Callback callback) {
    ReactApplicationContext context = getReactApplicationContext();

    if (mService.getState() == BluetoothService.STATE_CONNECTED) {
      printImageFromFilePath(context, imagePath,width);
      //printImageFromDrawable(context, 0);
