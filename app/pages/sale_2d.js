...
class sale extends Component {
  combineAmountTypeStorageKey = 'sale2d_combine_amount_type';
...
constructor(props) {
  ...
  getUserLabel(u) {
    ...
    const name = (u.UserName ?? u.Name ?? u.username ?? '').toString();
    if (no && name) return `${no} - ${name}`;
    return (no || name || String(u.UserID || u.UserId || '')) + '';
  }
  ...
  renderUsers() {
    if (this.state.user.length > 0) {
      return this.state.user.map((value, index) => {
        return (
          <Picker.Item label={this.getUserLabel(value)} value={value.UserID} key={index} />
        )
      })
    }
  }
...
_onPickUser = (userId) => {
                ...
                <View style={{flex:1,alignItems:"center",justifyContent:"center",backgroundColor:"#11030085"}}>
                  ...
                  <View style={{
                    ...
                    <View style={{height:height*0.06,width:width*0.75,marginHorizontal:width*0.05,flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
                        ...
                        <Text style={estyles.mmLargeText}>
                          {dal.numberWithCommas(((totalUnit*this.state.unitPrice)-(totalUnit*this.state.unitPrice*(parseInt(this.state.discount)/100))))}
                          </Text>
                    </View>
                        ...
                        <View style={{
                          ...
                          <FlatList
                            ...
                            renderItem={({item})=>(
                              ...
                              >
                                <Text style={{color:'#000',fontSize:16}}>{this.getUserLabel(item)}</Text>
                              </TouchableOpacity>
                            )}
                          />
                        </View>
                  ...
                  <Text style={{ fontSize: 16, fontWeight: 'bold', color: 'white' }}>OK</Text>
                    ...
                    ,alignItems:'center',marginHorizontal:width*0.05,flexDirection:'row'}}
                      ...
                      }} style={{flex:1,marginHorizontal:5,paddingVertical:5,backgroundColor:canSubmit?Color.Green:'#999',borderRadius:5,opacity:canSubmit?1:0.6,
                      ...
                      <TouchableOpacity disabled={!canSubmit} onPress={()=>{ if(!canSubmit){ Alert.alert(config.AppName,'Please select the user!'); return; }
                        ...
                        }} style={{marginRight:5,flex:1,paddingVertical:5,backgroundColor:canSubmit?Color.Green:'#999',borderRadius:5,opacity:canSubmit?1:0.6,
                          alignItems:'center',justifyContent:'center'}}>
  ...
  pressOk() {
    if (var_selected == 'num' && !this.state.sameUnit && !this.state.sameUnitR) {
      ...
      // plain 2-digit number + split unit format (e.g. 1000/500)
      if (var_chosen == '' && var_num.length == 2 && String(var_unit).indexOf('/') !== -1) {
        const arrNum = Array.from(var_num);
        const revNum = arrNum[1] + '' + arrNum[0];
        const unitParts = String(var_unit).split('/');
        const normalizePart = (v) => String(v == null ? '' : v).replace(/,/g, '').trim();
        const p1 = unitParts.length > 0 ? normalizePart(unitParts[0]) : '';
        const p2 = unitParts.length > 1 ? normalizePart(unitParts[1]) : '';
        const invalidFormat =
          unitParts.length !== 2 ||
          p1 === '' ||
          p2 === '' ||
          ...
          isNaN(Number(p2));
        if (invalidFormat) {
          this.refs.err_toast.show('Invalid unit format. Use number/number (e.g. 1000/500)', 1200);
          return;
        }
        const unitA = Number(p1);
        const unitB = Number(p2);
        ...
        UUIDGenerator.getRandomUUID().then((uuid) => {
          ...
          t.push({
            ...
            summary: var_num,
            discount: this.state.discount,
            GroupID: uuid,GroupID2: uuid,
            delete: false, showsummary: true
          });
          if (revNum !== var_num) {
            t.push({
              num: revNum,
          ...
          this.setState(
            {
              dataProvider: dataProvider.cloneWithRows(t.concat(__data)),
              data: this.state.data.concat(t),
              dataforSwap: t.concat(__data),
            },
            () => {
              this.refs.toast.show(var_num + ' = ' + unitA + (revNum !== var_num ? (' , ' + revNum + ' = ' + unitB) : ''), 1000);
              this.numViewComponent.setNativeProps({ style: { borderColor: "red" } })
              this.unitViewComponent.setNativeProps({ style: { borderColor: Color.defaultGray } })
        ...
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: "#11030085" }}>
          <View style={{
            ...
            <TouchableOpacity
              ...
              <Text style={[styles.mmmodalText, { marginLeft: 15 }]}>အပူး</Text>
            </TouchableOpacity>
      ...
      <View style={styles.container}>
        ...
        <Animated.View style={{
          ...
          <View style={{ width: width, height: height * 0.07, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
            ...
            <TouchableOpacity style={{ width: (width * 0.25) - 6.25, height: height * 0.06, justifyContent: 'center', alignItems: 'center', marginHorizontal: 5, backgroundColor: Color.lightGreen, borderRadius: 5 }}
              onPress={this.pressNum.bind(this, '/')}>
              <Text style={styles.mmText}>/</Text>
            </TouchableOpacity>
          </View>
          <View style={{ width: width, height: height * 0.07, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
